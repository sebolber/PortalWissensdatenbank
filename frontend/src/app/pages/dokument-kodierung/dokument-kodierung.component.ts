...
import { DocumentSuggestionDto } from '../../models/knowledge.model';
    ...
    <div class="card" *ngIf="documents.length > 0">
      ...
      <div class="doc-list">
        <div class="doc-item" *ngFor="let doc of documents"
             [class.doc-completed]="doc.status === 'COMPLETED'"
...
export class DokumentKodierungComponent implements OnInit, OnDestroy {
  ...
  documents: DocumentSuggestionDto[] = [];
  llmModels: LlmModelDto[] = [];
  selectedModelConfigId: string | null = null;
  selectedDocument: DocumentSuggestionDto | null = null;
  loading = false;
  refreshing = false;
  isDragOver = false;
  ...
  private uploadFile(file: File): void {
    this.chatApi.uploadDocumentSuggestion(file, this.selectedModelConfigId ?? undefined).subscribe({
      next: doc => { this.documents.unshift(doc); },
      error: err => { alert('Upload fehlgeschlagen: ' + (err.error?.error || err.message)); }
    });
  }

  startSuggestion(doc: DocumentSuggestionDto): void {
    doc.status = 'PROCESSING';
    this.chatApi.startDocumentSuggestion(doc.id).subscribe({
      error: err => {
        doc.status = 'ERROR';
  ...
  deleteDocument(doc: DocumentSuggestionDto): void {
    if (!confirm('Dokument "' + doc.fileName + '" wirklich loeschen?')) return;
    this.chatApi.deleteDocumentSuggestion(doc.id).subscribe({
      next: () => {
        this.documents = this.documents.filter(d => d.id !== doc.id);
        if (this.selectedDocument?.id === doc.id) this.selectedDocument = null;
      },
      error: err => { alert('Loeschen fehlgeschlagen: ' + (err.error?.error || err.message)); }
    });
  }
